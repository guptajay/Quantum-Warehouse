...
class Renderer():
    ...
    def __init__(self, fig_shape, shelves, spots, save_path= None, stats = None):
    ...
    def update(self, spots):
        ...
        # Saving
        if self.save:
            self.figure.savefig(os.path.join(self.save_path,'step_{}.png'.format(self.image_counter,dpi=100)))
            self.image_counter += 1

    def reset(self):
        for spot in list(self.spots.keys()):
            self.spots[spot].remove()
            self.spots.pop(spot)
    ...
    def draw_stats(self):
        if self.stats:
            ...
            x_margin,y_margin = .2, -.5
            stats = '\n'.join([ '{}:{}'.format(stat,self.stats[stat]) for stat in self.stats if stat in [
                'current_step', 'current_action']])
