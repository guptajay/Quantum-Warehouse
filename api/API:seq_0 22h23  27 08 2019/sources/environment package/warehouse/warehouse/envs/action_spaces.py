...
class AlleyAS(ActionSpace):
    '''
    Does not support long queue!
    '''
    
    def __init__(self, warehouse):
        super().__init__(warehouse)
        self.space = spaces.Discrete(self.n_alleys)

    def sample(self):
        return np.random.randint(self.n_alleys)

    def is_do_nothing(self,action):
        return action == self.n_alleys
