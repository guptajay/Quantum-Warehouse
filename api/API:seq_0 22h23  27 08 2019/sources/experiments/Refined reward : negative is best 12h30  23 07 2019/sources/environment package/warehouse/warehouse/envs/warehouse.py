...
from warehouse.envs.constants import initial_filled_spots_ratio, energy_cost_fn, packages, plt, min_filling_ratio, max_filling_ratio, spot_size
...
class Warehouse(gym.Env):
    ...
    def __init__(self, N=2, render = False, verbose = 1):
        self.seed=123
        ...
        self.n_alleys = 2*self.n_layers # we seperate the two orientations
    ...
    def retrieve_package(self, reward=False):
        ...
        if True:
            ...
            while spot is None:
                package = self.sample_package()
                spot = self.choose_spot_from_package(package)
            _package = spot.take() if not reward else spot.package
            assert _package.id == package.id
            reward = self.reward.compute_reward_for_retrieval(spot)
            # if self.verbose>0:
            #     print('Removed package with id {} and reward {}'.format(spot.id,reward))
            return reward
        return 0
    ...
    def create_valid_action_mask(self,state):
        mask = [ 1 if spot==0 else 0 for spot in state[:-self.n_queue] ]
        return mask
