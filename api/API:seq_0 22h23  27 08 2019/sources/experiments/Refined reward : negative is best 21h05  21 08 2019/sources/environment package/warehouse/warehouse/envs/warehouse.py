...
class Warehouse(gym.Env):
    ...
    def __init__(self, N=2, render = False, verbose = 1, api=True):
        self.seed=123
    ...
    def step(self, action):
        ...
        # Update API
        assert spot_size==1, "API not implemented from spots size > 1"
        spot_ids = []
        warehouse_package_ids = []
        for spot in self.spots:
            if len(spot.package)>0:
                spot_ids.append(spot.id)
                warehouse_package_ids.append(spot.package[0].id)
            else:
                spot_ids.append(spot.id)
                warehouse_package_ids.append(0)

        self.warehouse_api.to_csv(spot_ids=spot_ids, package_ids=warehouse_package_ids)
    ...
    def init_spawn_packages(self, ratio=.5, reset=False, verbose=1, api=False):
        ...
        if not api:
            ...
            mask, *_ = np.where(packages_ids>0)
