...
from warehouse.envs.constants import initial_fileld_spots_ratio, energy_cost_fn, packages, plt
from warehouse.envs.rendering import Renderer
from warehouse.envs.callbacks import StatisticsCallback, DescriptionCallback
...
class Warehouse(gym.Env):
    ...
    def __init__(self, N=2, render = False, verbose = 0):
        ...
        self.init_spawn_packages(ratio=initial_fileld_spots_ratio)
        ...
        self.callbacks = [self.statistics_callback, self.description_callback]
    ...
    def step(self, action):
        ...
        description = self.AS.describe(action)
        ...
        # Retrieve a package from the warehouse
        reward += self.retrieve_package()
        
        # Verbose option
        if self.verbose > 0:
            print('Step return is : `state`, {}, {}, {}'.format(reward,False,description))
        
        # Return the output
        return [self.state, reward, False, {'str':description}]


    def reset(self):

        # Remove and replace packages on shelves and queue
        self.init_spawn_packages(reset=True)
    ...
    def retrieve_package(self):
        ...
        if True:
            ...
            if spot is None:
                # there is no spot containing the requested package. Training has been stopped.
                return 0
            _package = spot.take()
