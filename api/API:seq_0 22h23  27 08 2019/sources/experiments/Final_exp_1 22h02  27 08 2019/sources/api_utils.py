...
from warehouse.envs.constants.basic_constants import initial_filled_spots_ratio, packages, spot_size
...
class APIWarehouseState(APIInputOuput):
    ...
    def from_csv(self, init=False):
        ...
        with open(os.path.join(path, "{}.csv".format(fname)), "r") as f:
            ...
            warehouse.update({"package_ids": [[int(float(y)) for y in x.split(',')[1:spot_size+1]] for x in lines[1:]]})

        return warehouse

    def to_csv(self, spot_ids, packages_ids):

        self.sequence_counter()
        fname = "{}_{}_{}.csv".format(self.fname, self.sequence_tag, self.sequence_counter.value)

        header = "Spot, " + ", ".join(["Package_{}".format(spot) for spot in range(1, spot_size+1)]) + ", " + ", " \
                                                                                                              "".join([
            "S/N_{}".format(spot) for spot in range(1, spot_size+1)]) + "\n"

        lines = [header] + ["{}, ".format(spot_id) + ", ".join([str(package_id) for package_id in packages_id]) + ", " \
            "999"*spot_size + "\n" for spot_id, packages_id in zip(spot_ids, packages_ids)]
...
class WarehouseFileGenerator(object):
    ...
    def generate(self):
        ...
        lines = [header] + ["{}, ".format(spot_id) + ", ".join([str(package_id) for package_id in packages_id]) + ", " \
            + ", ".join(["999"]*spot_size) + "\n" for spot_id, packages_id in zip(range(self.n_spots), [[int(x), 0, 0,
                                                                                                         0] for x in
                                                                                                        packages_])]
